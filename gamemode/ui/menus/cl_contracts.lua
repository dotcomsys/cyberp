...
local function buildContractsFrame(data)
    ...
    for _, c in ipairs(data or {}) do
        ...
        if c.target then
            local tgt = vgui.Create("DLabel", row)
            tgt:SetFont("CybeRp.Tiny")
            tgt:SetText("Target: " .. c.target)
            tgt:Dock(LEFT)
            tgt:DockMargin(12, 0, 0, 8)
            tgt:SizeToContents()
        end

        local prog = vgui.Create("DLabel", row)
        prog:SetFont("CybeRp.Tiny")
        local pct = math.floor((c.progress or 0) * 100)
        prog:SetText("Progress: " .. pct .. "%")
        prog:Dock(LEFT)
